...
bool UecSrc::_per_hop_rtt_normalization = false;
void UecSrc::setPerHopRttNormalization(bool enable) {
    _per_hop_rtt_normalization = enable;
}
...
void UecSrc::processAck(const UecAckPacket& pkt) {
    ...
    //or 2. the received ack is a retransmitted packet and local record shows this packet only gets retransmitted once. 
        ...
        if (!pkt.is_rts()) {
            update_base_rtt(raw_rtt, pkt.hop_count());
        }
        ...
        simtime_picosec exp_rtt = expected_rtt(pkt.hop_count());  // in dfp it depends on the path (different lengths)
        if (raw_rtt >= exp_rtt) {
            update_delay(raw_rtt, true, pkt.ecn_echo(), exp_rtt);
            delay = raw_rtt - exp_rtt;
        } else {
            delay = get_avg_delay();
        }
        ...
        if (pkt.is_probe_ack()){
            if (_probe_seqno == pkt.acked_psn()){
                ...
                if (_raw_rtt < _base_rtt){
                    ...
                    update_delay(_raw_rtt, true, pkt.ecn_echo(),_base_rtt);
                }
                if(_flow.flow_id() == _debug_flowid ){
...
void UecSrc::updateCwndOnNack_NSCC(bool skip, mem_b nacked_bytes, bool last_hop) {
    ...
    update_delay(_base_rtt + _network_rtt, true, true, _base_rtt);
...
void UecSrc::update_delay(simtime_picosec raw_rtt, bool update_avg, bool skip,  simtime_picosec expected_rtt){
    ...
    simtime_picosec delay = raw_rtt - expected_rtt;
    if(update_avg){

        if(skip == false && delay > _target_Qdelay){
            //_avg_delay = _delay_alpha * _base_rtt*0.25 + (1-_delay_alpha) * _avg_delay;
            _avg_delay = _delay_alpha * expected_rtt*0.25 + (1-_delay_alpha) * _avg_delay;
        }
        else{
            if (delay > 5* /*_base_rtt*/expected_rtt)
            {
                double r = 0.0125;
    ...
    if (_debug_src) {
        ...
        cout << "Update delay with sample " << timeAsUs(delay) << " avg is " << timeAsUs(_avg_delay) << " base rtt is " << _base_rtt << " expected rtt " << expected_rtt << endl;
    }
}
...
void UecSrc::processNack(const UecNackPacket& pkt) {
    ...
    /*if(raw_rtt >= _base_rtt) {
       ...
       simtime_picosec exp_rtt = expected_rtt(pkt.hop_count());
    if(raw_rtt >= exp_rtt) {
