...
unordered_map<BaseQueue*,uint32_t> IncFatTreeSwitch::_port_flow_counts;

IncFatTreeSwitch::IncFatTreeSwitch(EventList& eventlist, string s, switch_type t, uint32_t id,simtime_picosec delay, IncFatTreeTopology* ft): Switch(eventlist, s) {
    _id = id;
    _type = t;
...
IncFatTreeSwitch::~IncFatTreeSwitch() {
    delete _pipe;
    delete _fib;
}

void IncFatTreeSwitch::receivePacket(Packet& pkt){
...
void IncFatTreeSwitch::addHostPort(int addr, int flowid, PacketSink* transport_port){
    Route* rt = new Route();
...
uint32_t IncFatTreeSwitch::adaptive_route_p2c(vector<FibEntry*>* ecmp_set, int8_t (*cmp)(FibEntry*,FibEntry*)){
...
uint32_t IncFatTreeSwitch::adaptive_route(vector<FibEntry*>* ecmp_set, int8_t (*cmp)(FibEntry*,FibEntry*)){
...
uint32_t IncFatTreeSwitch::replace_worst_choice(vector<FibEntry*>* ecmp_set, int8_t (*cmp)(FibEntry*,FibEntry*),uint32_t my_choice){
...
int8_t IncFatTreeSwitch::compare_pause(FibEntry* left, FibEntry* right){
...
int8_t IncFatTreeSwitch::compare_flow_count(FibEntry* left, FibEntry* right){
    Route * r1= left->getEgressPort();
    assert(r1 && r1->size()>1);
...
int8_t IncFatTreeSwitch::compare_queuesize(FibEntry* left, FibEntry* right){
    Route * r1= left->getEgressPort();
    assert(r1 && r1->size()>1);
...
int8_t IncFatTreeSwitch::compare_bandwidth(FibEntry* left, FibEntry* right){
    Route * r1= left->getEgressPort();
    assert(r1 && r1->size()>1);
    BaseQueue* q1 = dynamic_cast<BaseQueue*>(r1->at(0));
    Route * r2= right->getEgressPort();
...
int8_t IncFatTreeSwitch::compare_pqb(FibEntry* left, FibEntry* right){
...
int8_t IncFatTreeSwitch::compare_pq(FibEntry* left, FibEntry* right){
    //compare pause, queuesize, bandwidth.
    int8_t p = compare_pause(left, right);
...
int8_t IncFatTreeSwitch::compare_qb(FibEntry* left, FibEntry* right){
    //compare pause, queuesize, bandwidth.
...
int8_t IncFatTreeSwitch::compare_pb(FibEntry* left, FibEntry* right){
    //compare pause, queuesize, bandwidth.
    int8_t p = compare_pause(left, right);
...
void IncFatTreeSwitch::permute_paths(vector<FibEntry *>* uproutes) {
...
IncFatTreeSwitch::routing_strategy IncFatTreeSwitch::_strategy = IncFatTreeSwitch::NIX;
uint16_t IncFatTreeSwitch::_ar_fraction = 0;
uint16_t IncFatTreeSwitch::_ar_sticky = IncFatTreeSwitch::PER_PACKET;
simtime_picosec IncFatTreeSwitch::_sticky_delta = timeFromUs((uint32_t)10);
double IncFatTreeSwitch::_ecn_threshold_fraction = 0.2;
double IncFatTreeSwitch::_speculative_threshold_fraction = 0.2;
int8_t (*IncFatTreeSwitch::fn)(FibEntry*,FibEntry*)= &IncFatTreeSwitch::compare_queuesize;
uint16_t IncFatTreeSwitch::_trim_size = 64;
bool IncFatTreeSwitch::_disable_trim = false;

Route* IncFatTreeSwitch::getNextHop(Packet& pkt, BaseQueue* ingress_port){
    ...
    if (available_hops){
        ...
        if (available_hops->size()>1)
            ...
            case ADAPTIVE_ROUTING:
                ...
                if (_ar_sticky==IncFatTreeSwitch::PER_PACKET){
                    ecmp_choice = adaptive_route(available_hops,fn); 
                } 
                else if (_ar_sticky==IncFatTreeSwitch::PER_FLOWLET){     
        ...
        if ( _ft->cfg().HOST_POD_SWITCH(pkt.dst()) == _id) { 
            ...
            if (_uproutes)
                ...
                for (uint32_t k=agg_min; k<=agg_max;k++){
                    ...
                    /*
                      IncFatTreeSwitch* next = (IncFatTreeSwitch*)_ft->queues_nlp_nup[_id][k]->getRemoteEndpoint();
            ...
            if (_uproutes)
                ...
                for (uint32_t l = 0; l <  uplink_bundles ; l++) {
                    ...
                    for (uint32_t b = 0; b < _ft->cfg().bundlesize(CORE_TIER); b++) {
                        ...
                        /*
                          IncFatTreeSwitch* next = (IncFatTreeSwitch*)_ft->queues_nup_nc[_id][k]->getRemoteEndpoint();
