...
simtime_picosec calculate_rtt(IncFatTreeTopologyCfg* t_cfg, linkspeed_bps host_linkspeed) { 
    /*
...
uint32_t calculate_bdp_pkt(IncFatTreeTopologyCfg* t_cfg, linkspeed_bps host_linkspeed) {
...
int main(int argc, char **argv) {
    ...
    IncFatTreeSwitch::sticky_choices ar_sticky = IncFatTreeSwitch::PER_PACKET;
...
//            cout << "fair_decrease disabled" << endl;
            ...
            if (!strcmp(argv[i+1],"packet"))
                ar_sticky = IncFatTreeSwitch::PER_PACKET;
            else if (!strcmp(argv[i+1],"flow"))
                ar_sticky = IncFatTreeSwitch::PER_FLOWLET;
            else  {
            ...
            if (!strcmp(argv[i+1], "ecmp_host")) {
                ...
                IncFatTreeSwitch::set_strategy(IncFatTreeSwitch::ECMP);
                ...
                IncFatTreeSwitch::set_strategy(IncFatTreeSwitch::RR_ECMP);
                ...
                route_strategy = ECMP_FIB;
                path_entropy_size = 1;
                IncFatTreeSwitch::set_strategy(IncFatTreeSwitch::ADAPTIVE_ROUTING);
            } else if (!strcmp(argv[i+1], "ecmp_host_ar")) {
                route_strategy = ECMP_FIB;
                IncFatTreeSwitch::set_strategy(IncFatTreeSwitch::ECMP_ADAPTIVE);
                //the stuff below obsolete
                //IncFatTreeSwitch::set_ar_fraction(atoi(argv[i+2]));
                ...
                route_strategy = ECMP_FIB;
                path_entropy_size = 1;
                IncFatTreeSwitch::set_strategy(IncFatTreeSwitch::RR);
            }
            i++;
        } else {
    ...
    */
    ...
    IncFatTreeSwitch::_ar_sticky = ar_sticky;
    IncFatTreeSwitch::_sticky_delta = timeFromUs(ar_sticky_delta);
    IncFatTreeSwitch::_ecn_threshold_fraction = ecn_thresh;
    IncFatTreeSwitch::_disable_trim = disable_trim;
    IncFatTreeSwitch::_trim_size = trimsize;
    ...
    auto conns = std::make_unique<IncConnectionMatrix>(no_of_nodes);
    ...
    unique_ptr<IncFatTreeTopologyCfg> topo_cfg;
    if (topo_file) {
        topo_cfg = IncFatTreeTopologyCfg::load(topo_file, memFromPkt(queuesize_pkt), qt, snd_type);
        ...
        topo_cfg = make_unique<IncFatTreeTopologyCfg>(tiers, no_of_nodes, linkspeed, memFromPkt(queuesize_pkt),
    ...
    vector<unique_ptr<IncFatTreeTopology>> topo;
    topo.resize(planes);
    for (uint32_t p = 0; p < planes; p++) {
        topo[p] = make_unique<IncFatTreeTopology>(topo_cfg.get(), qlf, &eventlist, nullptr);
        
        if (log_switches) {
            topo[p]->add_switch_loggers(logfile, logtime);
        }
       IncFatTreeTopology* ft = topo[p].get();
