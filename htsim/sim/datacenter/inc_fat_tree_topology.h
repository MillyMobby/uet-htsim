...
class IncFatTreeTopology;


class IncFatTreeTopologyCfg {
friend class IncFatTreeTopology;
friend class IncTopology;
friend std::ostream &operator<<(std::ostream &os, IncFatTreeTopologyCfg const &m);
public:
    IncFatTreeTopologyCfg(queue_type q, queue_type snd);
    IncFatTreeTopologyCfg(istream& file, mem_b queue_size,
                       queue_type q, queue_type snd);
    IncFatTreeTopologyCfg(uint32_t tiers, uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b queuesize, 
                       simtime_picosec latency, simtime_picosec switch_latency, 
                       queue_type q, queue_type snd);
    IncFatTreeTopologyCfg(uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b queuesize,
                       queue_type q);      
    IncFatTreeTopologyCfg(uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b queuesize,
                       queue_type q, uint32_t num_failed);      
    IncFatTreeTopologyCfg(uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b queuesize, uint32_t fail,
                       queue_type q, queue_type snd);

    static unique_ptr<IncFatTreeTopologyCfg> load(string filename, mem_b queuesize, queue_type q_type, queue_type sender_q_type);
    /*
...
public:
    IncFatTreeTopology(const IncFatTreeTopologyCfg* cfg,
                    QueueLoggerFactory* logger_factory,
                    EventList* ev,
                    FirstFit * fit);
    ~IncFatTreeTopology() override;
    ...
    const IncFatTreeTopologyCfg& cfg() { return *_cfg; };
protected:
    const IncFatTreeTopologyCfg* _cfg;
