...
#define N_HOSTS_FOR_GROUP 4

class FatTreeTopologyMg;


class FatTreeTopologyCfgMg {
friend class FatTreeTopologyMg;
friend std::ostream &operator<<(std::ostream &os, FatTreeTopologyCfgMg const &m);
public:
    FatTreeTopologyCfgMg(queue_type q, queue_type snd);

    FatTreeTopologyCfgMg(istream& file, mem_b queue_size,
                       queue_type q, queue_type snd);

    FatTreeTopologyCfgMg(uint32_t tiers, uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b queuesize, 
                       simtime_picosec latency, simtime_picosec switch_latency, 
                       queue_type q, queue_type snd);

    FatTreeTopologyCfgMg(uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b queuesize,
                       queue_type q);     

    FatTreeTopologyCfgMg(uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b queuesize,
                       queue_type q, uint32_t num_failed);  
                           
    FatTreeTopologyCfgMg(uint32_t no_of_nodes, linkspeed_bps linkspeed, mem_b queuesize, uint32_t fail,
                       queue_type q, queue_type snd);

    static unique_ptr<FatTreeTopologyCfgMg> load(string filename, mem_b queuesize, queue_type q_type, queue_type sender_q_type);
    /*
...
class FatTreeTopologyMg: public Topology{
public:
    FatTreeTopologyMg(const FatTreeTopologyCfgMg* cfg,
                    QueueLoggerFactory* logger_factory,
                    EventList* ev,
                    FirstFit * fit);
    ~FatTreeTopologyMg() override;
    ...
    vector< vector< vector<BaseQueue*> > > queues_host_host;
    vector< vector< vector<Pipe*> > > pipes_host_host;
    ...
    bool has_host_host_link(int src, int dst);
    ...
    const FatTreeTopologyCfgMg& cfg() { return *_cfg; };
private:
    const FatTreeTopologyCfgMg* _cfg;
