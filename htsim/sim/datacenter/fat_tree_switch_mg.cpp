...
unordered_map<BaseQueue*,uint32_t> FatTreeSwitchMg::_port_flow_counts;

FatTreeSwitchMg::FatTreeSwitchMg(EventList& eventlist, string s, switch_type t, uint32_t id,simtime_picosec delay, FatTreeTopologyMg* ft): Switch(eventlist, s) {
    _id = id;
    _type = t;
...
FatTreeSwitchMg::~FatTreeSwitchMg() {
    delete _pipe;
    delete _fib;
}

void FatTreeSwitchMg::receivePacket(Packet& pkt){
...
void FatTreeSwitchMg::addHostPort(int addr, int flowid, PacketSink* transport_port){
    Route* rt = new Route();
...
uint32_t FatTreeSwitchMg::adaptive_route_p2c(vector<FibEntry*>* ecmp_set, int8_t (*cmp)(FibEntry*,FibEntry*)){
...
uint32_t FatTreeSwitchMg::adaptive_route(vector<FibEntry*>* ecmp_set, int8_t (*cmp)(FibEntry*,FibEntry*)){
...
uint32_t FatTreeSwitchMg::replace_worst_choice(vector<FibEntry*>* ecmp_set, int8_t (*cmp)(FibEntry*,FibEntry*),uint32_t my_choice){
...
int8_t FatTreeSwitchMg::compare_pause(FibEntry* left, FibEntry* right){
...
int8_t FatTreeSwitchMg::compare_flow_count(FibEntry* left, FibEntry* right){
    Route * r1= left->getEgressPort();
    assert(r1 && r1->size()>1);
...
int8_t FatTreeSwitchMg::compare_queuesize(FibEntry* left, FibEntry* right){
    Route * r1= left->getEgressPort();
    assert(r1 && r1->size()>1);
...
int8_t FatTreeSwitchMg::compare_bandwidth(FibEntry* left, FibEntry* right){
    Route * r1= left->getEgressPort();
    assert(r1 && r1->size()>1);
    BaseQueue* q1 = dynamic_cast<BaseQueue*>(r1->at(0));
    Route * r2= right->getEgressPort();
...
int8_t FatTreeSwitchMg::compare_pqb(FibEntry* left, FibEntry* right){
...
int8_t FatTreeSwitchMg::compare_pq(FibEntry* left, FibEntry* right){
    //compare pause, queuesize, bandwidth.
    int8_t p = compare_pause(left, right);
...
int8_t FatTreeSwitchMg::compare_qb(FibEntry* left, FibEntry* right){
    //compare pause, queuesize, bandwidth.
...
int8_t FatTreeSwitchMg::compare_pb(FibEntry* left, FibEntry* right){
    //compare pause, queuesize, bandwidth.
    int8_t p = compare_pause(left, right);
...
void FatTreeSwitchMg::permute_paths(vector<FibEntry *>* uproutes) {
...
FatTreeSwitchMg::routing_strategy FatTreeSwitchMg::_strategy = FatTreeSwitchMg::NIX;
uint16_t FatTreeSwitchMg::_ar_fraction = 0;
uint16_t FatTreeSwitchMg::_ar_sticky = FatTreeSwitchMg::PER_PACKET;
simtime_picosec FatTreeSwitchMg::_sticky_delta = timeFromUs((uint32_t)10);
double FatTreeSwitchMg::_ecn_threshold_fraction = 0.2;
double FatTreeSwitchMg::_speculative_threshold_fraction = 0.2;
int8_t (*FatTreeSwitchMg::fn)(FibEntry*,FibEntry*)= &FatTreeSwitchMg::compare_queuesize;
uint16_t FatTreeSwitchMg::_trim_size = 64;
bool FatTreeSwitchMg::_disable_trim = false;

Route* FatTreeSwitchMg::getNextHop(Packet& pkt, BaseQueue* ingress_port){
    ...
    if (available_hops){
        ...
        if (available_hops->size()>1)
            ...
            case NIX:
                abort();
            case ECMP:
                ecmp_choice = freeBSDHashMg(pkt.flow_id(),pkt.pathid(),_hash_salt) % available_hops->size();
                break;
            case ADAPTIVE_ROUTING:
                ...
                if (_ar_sticky==FatTreeSwitchMg::PER_PACKET){
                    ecmp_choice = adaptive_route(available_hops,fn); 
                } 
                else if (_ar_sticky==FatTreeSwitchMg::PER_FLOWLET){     
                    if (_flowlet_maps.find(pkt.flow_id())!=_flowlet_maps.end()){
                        FlowletInfoMg* f = _flowlet_maps[pkt.flow_id()];
                        ...
                        _flowlet_maps[pkt.flow_id()] = new FlowletInfoMg(ecmp_choice,eventlist().now());
                    }
                }
            ...
            case RR_ECMP:
                ...
                else ecmp_choice = freeBSDHashMg(pkt.flow_id(),pkt.pathid(),_hash_salt) % available_hops->size();
        ...
        if ( _ft->cfg().HOST_POD_SWITCH(pkt.dst()) == _id) { 
            ...
            if (_uproutes)
                ...
                for (uint32_t k=agg_min; k<=agg_max;k++){
                    ...
                    /*
                      FatTreeSwitchMg* next = (FatTreeSwitchMg*)_ft->queues_nlp_nup[_id][k]->getRemoteEndpoint();
            ...
            if (_uproutes)
                ...
                for (uint32_t l = 0; l <  uplink_bundles ; l++) {
                    ...
                    for (uint32_t b = 0; b < _ft->cfg().bundlesize(CORE_TIER); b++) {
                        ...
                        /*
                          FatTreeSwitchMg* next = (FatTreeSwitchMg*)_ft->queues_nup_nc[_id][k]->getRemoteEndpoint();
